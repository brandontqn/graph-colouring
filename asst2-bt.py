...
def recursiveBacktrackSearch(solution, n, k, G):
    ...
    for node in G[len(solution):]: # CANDIDATE LINE TO IMPLEMENT MRV AND DEGREE HEURISTIC
        col = selectUnassignedColour(node[0], solution, k, G) # find acceptable colour assignment for node
        if(col): # there are available, acceptable assignments
            solution.append((node[0], col)) # append (node, col) to our solution
            ...
            if(result != []): # moving ahead does not result in failure
                return result
            solution.remove((node[0], col)) # moving ahead results in failure, remove current node+colour from solution *****
